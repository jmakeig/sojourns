...
QueryBuilder.prototype = {
   ...
   * ml.where()
   *   .page(15)  // Get only the top 15 values, based on the values ordering below
   *   .values("air_date", function(min, max) { // The ranges callback gives you access to the min and max value for the current set of input documents filtered by the `.where()`.
   *     return ml.buckets.byMonth(min, max, 1);  // Given those absolute ranges
