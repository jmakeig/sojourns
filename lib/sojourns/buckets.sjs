...
function monthBuckets(start, end, dateStart /* 1-31 */) {
  ...
  var out = [];
  do {
    out.push(start.clone().toDate());
  } while(start.add(1, 'months') <= end)
  return out;
}
/*
 * Week-long buckets fully containing the start and end dates. Optionally set where to 
 * start counting in the week (0: Sun ... 6: Sat).
 * @param {Date|Value} start - Start date
 * @param {Date|Value} end - End date
 * @param {number} [dayStart]  - The day of the week to start on. Defaults to `start.day()`.
 */
