...
QueryBuilder.prototype = {
   ...
   * Examples:
   *   .orderBy("random"): Random results. Pair with .page() for sampling.
   *   .orderBy({score: true}): Score, descending. The default if no arguments are specified or .where() is never called.
   *   .orderBy([{property: "age", direction: "ascending" }, {score: true}]): JSON property age ascending then by score descending for ties.
