...
QueryBuilder.prototype = {
  ...
  */
  /*
  function* values2(rangeIndexes, bounds, options) {
    if('undefined' === typeof rangeIndexes) { return undefined; }
    
    // Should we union multiple range indexes together or should they be treated as tuple values?
    var isUnion = ('union' in rangeIndexes);
    // Get the list of requested range indexes
    var rangeIndexes = [].concat.apply([], [(isUnion ? rangeIndexes.union : rangeIndexes)]);
    // Allow passing cts.references or strings. Strings are turned into cts.jsonPropertyReferences
    rangeIndexs = rangeIndexes.map(function(ref) {
      if('string' === typeof ref) {
        return cts.jsonPropertyReference(ref);
      } else if(ref instanceof Value) {
        return ref;
      }
      throw new Error('Neither a string nor a Value');
    });
    
    if(rangeIndexes.length > 1 && !isUnion) {
      cts.valueTuples(rangeIndexes, options, this.getQuery());
    }
  },
  */
  ...
  values: function* (rangeIndexes /* String|String[],  */, ranges, options /* {order: "frequency|item", frequency: "fragment|item", direction: "ascending|descending", limit: N, skip: N, sample: N, truncate: N, score: "logtfidf|logtf|simple|random|zero"}, forests: ["name"], qaulityWeight: N } */) {
    ...
    var isUnion = 'string' !== typeof rangeIndexes && 'union' in rangeIndexes;
    ...
    for(var opt in options) {
      switch(opt) {
        ...
        case "truncate":  // Include only values from the first N fragments after skip selected by the cts:query. This option also affects the number of fragments selected by the cts:query to calculate frequencies.
          ...
          break;
        case "empties": // This is handled below because it only applies toe cts.valueRanges()
        case "concurrent":
        case "map":
        case "timezone":
          // Unsupported
    ...
    if(1 === rangeIndexes.length || isUnion) { // FIXME: cts.values() supports multiple inputs. These are union-ed together, which is pretty powerful.
      if(!ranges || (!Array.isArray(ranges) && !(ranges instanceof Function))) {
        ...
        // If ranges is a function call it, passing back the min and max values.
    ...
    for(var value of itr) {
      ...
      if(value instanceof Value && !(value instanceof Node) && (rangeIndexes.length === 1 || isUnion)) {  // cts.values
        ...
        // }
        value = value.valueOf();
      } else if(value instanceof Value && rangeIndexes.length > 1 && !isUnion) { // cts.tuples
        // cts.valueTuples
        // {
        //   "item": [
        //     "1987-04-07",  // air_date
